"""
an implement of BP-MLL loss function using PyTorch
"""
...
class BPMLLLoss(torch.nn.Module):
    ...
    def forward(self, c: Tensor, y: Tensor) -> Tensor:
        ...
        return torch.mean(torch.tensor(1.0) / torch.mul(y_norm, y_bar_norm) * self.pairwise_sub_exp(y, y_bar, c))
...
def hamming_loss(c: Tensor, y: Tensor, threshold=0.5) -> Tensor:
    """
    ...
    :param threshold: the minimum possibility that will be considered as positive
    :return: Scalar
    """
    assert 0 <= threshold <= 1, "threshold should be between 0 and 1"
    p, q = c.size()
    return 1.0 / (p * q) * ((c > threshold).bool() ^ y.bool()).float().sum()
